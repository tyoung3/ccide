...
TestIt() {
	...
	[ -z $TIN ] && TIN=/dev/null 
	./$X $TOPT  < $TIN >$X.output 	\
	&& diff -b -q $TDIR/$T.X $X.output  >/dev/null

}

CheckExpand() {
	diff -b $* >/dev/null && Success "Generate $1" 	\
			|| Fail "Generate" $1  
}
...
TestCase() {
	...
	return  ;  # Execution not really necessary, now
	TINPUT=`ls $TDIR/$CASE*.X 2>/dev/null`
	if [ $? == 0 ]; then 
		T=`basename $TINPUT .X`  	\
		&& $MAKE $CASE 			\
		&& X=$CASE 			\
...
MakeAM() {
	...
	echo "# scripts/check.sh.  Generated from check.sh.in by configure. "						   > $TMPLT
	echo "# scripts/check.sh.  Generated from check.sh.in by configure. "						   > $PTMPLT
	...
	echo "EXTRA_DIST = autogen.sh src/ccide.1 ccide.spec src/ccidew ccide.spec.in ChangeLog CYGWIN-PATCHES/setup.hint.in scripts/ccide.d.in \\" 	 								   >> $TMPLT
	...
	MakeEntry "	scripts/check.sh scripts/Makefile scripts/Makefile.in scripts/ccide.d.in scripts/check.sh.in"
...
VERSION=0.7.0-0
[ -d ../tests ] && pushd ../tests  >/dev/null
[ -d ./tests  ] && pushd ./tests  >/dev/null
#[ -f ../tests/threeway.X ] || echo cannot find ../tests/threeway.X 

TDIR=../tests
[ -f $TDIR/threeway.X ] || TDIR=../../tests
...
case $1 in
	...
	checkgen)shift; [ -z $1 ] && Usage 
		...
		;;
	*) 	[ -z $1 ] && Usage 
			# MALLOC_CHECK clutters up make check too much with the messsage:   "malloc: using debugging hooks"
		export CCIDE_SAVELANG=$LANG  
	   	for T in $*; do 
			export LANG=en_US.UTF-8
			TestCase $T 
	  	done
		export LANG=$CCIDE_SAVELANG
	        Summarize
	   ;;
esac
