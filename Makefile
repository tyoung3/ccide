...
CONFIG_CLEAN_FILES = autogen.sh scripts/check.sh scripts/ccide.d \
	ccide.spec disp_pkg.mk extra_dist.mk CYGWIN-PATCHES/setup.hint \
	scripts/Makefile
...
am__DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/autogen.sh.in \
	$(srcdir)/ccide.spec.in $(srcdir)/disp_pkg.mk \
	$(srcdir)/disp_pkg.mk.in $(srcdir)/extra_dist.mk \
	$(srcdir)/extra_dist.mk.in \
	$(top_srcdir)/CYGWIN-PATCHES/setup.hint.in \
	$(top_srcdir)/scripts/Makefile.in \
	$(top_srcdir)/scripts/ccide.d.in \
	$(top_srcdir)/scripts/check.sh.in ABOUT-NLS AUTHORS COPYING \
	ChangeLog INSTALL NEWS README.md TODO.md compile config.guess \
...
YACC = bison -y
YFLAGS = 
abs_builddir = /usr/local/src/ccide
abs_srcdir = /usr/local/src/ccide
...
# scripts/check.sh.  Generated from check.sh.in by configure. 
dist_pkgdata_DATA = m4/ccide-C.m4  m4/ccide-CC.m4 m4/ccide-C++.m4\
	m4/ccide-BASH.m4 m4/ccide-BASIC.m4 m4/ccide-QB.m4 \
	src/Makefile.in  src/ccide.pod.in src/ccideconfig.h.in \
	src/ccidemain.h src/ccide.h src/gettext.h src/parse.h $(srcdir)/src/ccidelex.l \
	src/ccideinline.c  src/ccidelex.c  src/ccidemain.c  src/ccideparse.c src/ccideparse.y  src/cciderunx.c \
	tests/f1 tests/f2 tests/f3 tests/f4 tests/if2rpn.txt tests/tryit.txt tests/Makefile \
	...
	./tests/undo.c.d \
	./tests/version.c.d \
	m4/ccide-VB.m4 m4/ccide-JAVA.m4 m4/ccide-CS.m4 m4/ccide-EX.m4
...
am--refresh: Makefile
	@:
$(srcdir)/Makefile.in: # $(srcdir)/Makefile.am $(srcdir)/extra_dist.mk $(srcdir)/disp_pkg.mk $(am__configure_deps)
	@for dep in $?; do \
...
$(srcdir)/extra_dist.mk $(srcdir)/disp_pkg.mk $(am__empty):
...
$(am__aclocal_m4_deps):
autogen.sh: $(top_builddir)/config.status $(srcdir)/autogen.sh.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
scripts/check.sh: $(top_builddir)/config.status $(top_srcdir)/scripts/check.sh.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
scripts/ccide.d: $(top_builddir)/config.status $(top_srcdir)/scripts/ccide.d.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
ccide.spec: $(top_builddir)/config.status $(srcdir)/ccide.spec.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
disp_pkg.mk: $(top_builddir)/config.status $(srcdir)/disp_pkg.mk.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
extra_dist.mk: $(top_builddir)/config.status $(srcdir)/extra_dist.mk.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
CYGWIN-PATCHES/setup.hint: $(top_builddir)/config.status $(top_srcdir)/CYGWIN-PATCHES/setup.hint.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
