...
GenInLineCode (char *s)
{
  ...
  printf ("ccide-%s-%s\n", VERSION, RELEASE);
  J_ ("ccide is Copyright (C) 2002-2004,2010,2012;  Thomas W. Young, e-mail:  ccide@twyoung.com") J_ ("The code generated by ccide is covered by the same license as the source ") J_ ("code(decision table) from which it is derived. If you created the source, ") J_ ("you are free to do anything you like with the generated code,") J_ ("including incorporating it into or linking it with proprietary software. ") J ("*/") printf ("static int %s_group=1;", pPrefixLc);
  J ("")
    J ("#ifndef UINT_MAX")
    J ("#include \"limits.h\"")
    J ("#endif ")
...
GenSkeleton (char *s)
{
  ...
  printf (_("%s Skeleton Program Generated by ccide-%s.%s %s %s\n"),
  ...
  J ("")
    ...
    J (" ") J ("#include <stdlib.h>") J ("#include <stdio.h>")
    J ("#include <assert.h>") printf ("#ifdef %s_LIB\n", pPrefix);
  printf ("#include <%s.h>\n", pPrefixLc);
  J ("#else") printf ("%s %s_INLINE_CODE:%s\n", pComment, pPrefix, pEcomment);
  printf ("#endif %s End #ifdef %s_LIB %s", pComment, pPrefix, pEcomment);
  J ("\nstatic void A(int n) {")
    printf ("\tprintf((const char*)\"%s\\n\",n);\n", "%i");

  J ("}\n") J ("int main(int argc, char **argv) {") i = skelsize;
  while ((1 << logsize) < i)
    logsize++;
...
#define bar "__"
  ...
  J ("")
    J ("	return 0;")
    J ("}")
    J ("") printf (_("%sEnd of Skeleton Program.%s\n"), pComment, pEcomment);
...
Usage ()
{
  ...
  J ("Usage:")
    ...
    J ("       | ccidew --help")
    J
    ("       | ccidew [-a] [-b] [-c COLUMNSIZE] [-d STUBSTRING1 STUBSTRING2] [-e] [-l] ")
    J
    ("               [-L LANG] [-m4] [-n] [-p PREFIX] [-q LEFTQUOTE RIGHTQUOTE]")
    J ("	          [-s SKELSIZE][-t] [-u] [-x] < STDIN > STDOUT") RC =
    1;
  exit (1);
}
